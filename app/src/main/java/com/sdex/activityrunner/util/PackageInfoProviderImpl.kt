...
class PackageInfoProviderImpl(
    ...
    override fun getApplication(packageName: String): ApplicationModel? = try {
    ...
    override fun getActivities(packageName: String): List<ActivityModel> {
        val packageInfo = getPackageInfo(packageName)
        val applicationInfo = packageInfo.applicationInfo
            ?: return emptyList()
    ...
    override fun getChangedPackages(lastSequenceNumber: Int): ChangedPackages? =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            packageManager.getChangedPackages(lastSequenceNumber)?.let {
                ChangedPackages(
                    sequenceNumber = it.sequenceNumber,
                    packageNames = it.packageNames.toList(),
                )
            }
        } else {
            null
